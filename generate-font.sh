...
if [[ -z $outputFont || -z $inputFont || -z $textFile ]]; then
    ...
    if [[ -z $webFont ]]; then
        ...
        docker run -e LC_CTYPE=zh_TW.UTF-8 -w /root/font-generator -v ${currentPath}:/root/font-generator -it peter1209/fontforge /root/font-generator/bin/generate-font.py $webFont $inputFont $outputFont $textFile
    fi
fi
